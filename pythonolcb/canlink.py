...
class CanLink(LinkLayer) : 
    ...
    def receiveListener(self, frame) : # CanFrame
        match self.decodeControlFrameFormat(frame) :
            ...
            case ControlFrame.LinkCollision, ControlFrame.LinkError :
                logging.warning("Unexpected error report \(frame.header, format:.hex(minDigits: 8))")
            ...
            case ControlFrame.EIR0, ControlFrame.EIR1, ControlFrame.EIR2, ControlFrame.EIR3 :
                pass   # ignored upon receipt
            case ControlFrame.Data :
                self.handleReceivedData(frame)
            case ControlFrame.UnknownFormat :
                logging.warning("Unexpected CAN header 0x{:08X}".format(frame.header))
    ...
    def defineAndReserveAlias(self):
        ...
        self.link.sendCanFrame( CanFrame(ControlFrame.AMD.value, self.localAlias, self.localNodeID.toArray()) )
        ...
        self.link.sendCanFrame( CanFrame(ControlFrame.AME.value, self.localAlias) )
    ...
    def handleReceivedCID(self, frame) : # CanFrame
        ...
        if (frame.header & 0xFFF) != self.localAlias : return #    no match
        #    send an RID in response
        self.link.sendCanFrame( CanFrame(ControlFrame.RID.value, self.localAlias) )
    
    def handleReceivedRID(self, frame) : # CanFrame
        if (self.checkAndHandleAliasCollision(frame)) : return 
    ...
    def handleReceivedAME(self, frame) : # CanFrame
        if (self.checkAndHandleAliasCollision(frame)) : return 
        if (self.state != LinkLayer.State.Permitted) : return 
        #    check node ID
        ...
        if (self.localNodeID == matchNodeID) :
            #    matched, send RID
            returnFrame = CanFrame( ControlFrame.AMD.value, self.localAlias, self.localNodeID.toArray())
    ...
    def handleReceivedData(self, frame) : # CanFrame
        ...
        try : 
            ...
            if mti == MTI.Verified_NodeID :
                ...
                logging.info("Verified_NodeID from unknown source alias: {}, continue with observed ID {}".format(frame, sourceID))
            else :
                sourceID = NodeID(self.nextInternallyAssignedNodeID)
                self.nextInternallyAssignedNodeID += 1
                logging.warning("message from unknown source alias: {}, continue with created ID {}".format(frame, sourceID))
        ...
        if (frame.header & 0x008_000 != 0 or (dgCode >= 0x00A_000_000 and dgCode <= 0x00F_000_000)) :  #    Addressed bit is active 1
            ...
            if (dgCode >= 0x00A_000_000 and dgCode <= 0x00F_000_000) :
                ...
                if mapped != None :
                    ...
                    logging.warning("message from unknown dest alias: {}, continue with {}", format(frame, destID))
                    #    register that internally-generated nodeID-alias association
                ...
                if (dgCode == 0x00A_000_000 or dgCode == 0x00B_000_000 ) :
                    ...
                    if not key in self.accumulator :
                        #    have not-start frame, but never started
                        logging.warning("Dropping non-start datagram frame without accumulation started: \(frame, privacy: .public)")
                ...
                destAlias = 0
                if (len(frame.data) > 0) : destAlias |= (frame.data[0] & 0x0F ) << 8  #    rm f bits
                if (len(frame.data) > 1) : destAlias |= (frame.data[1] & 0xFF ) 
                try :
                    mapped = self.aliasToNodeID[destAlias]
                    destID = mapped
                except :
                    destID = NodeID(self.nextInternallyAssignedNodeID)
                    logging.warning("message from unknown dest alias: 0x{:04X}, continue with 0x{}".format(destAlias, destID))
                    #    register that internally-generated nodeID-alias association
                ...
                if (frame.data[0] & 0x20 == 0) :
                    ...
                    if not key in self.accumulator :
                        #    have not-start frame, but never started
                        logging.warning("Dropping non-start frame without accumulation started: {}".format(frame))
                        return #    early return to stop processing of this grame
    ...
    def sendMessage(self, msg) :
        ...
        if msg.mti == MTI.Datagram :
            ...
            try :
                ...
                logging.warning("Did not know source = {} on datagram send".format(msg.source))
            
            try :
                dddAlias = self.nodeIdToAlias[msg.destination]
                header |= ((dddAlias) & 0xFFF) << 12
            except :
                logging.warning("Did not know destination = {} on datagram send".format(msg.source))
            ...
            if len(msg.data) <= 8 :
                ...
                if (len(dataSegments) >= 3) :
                    for index in range(1, len(dataSegments) - 2 + 1) : #    upper limit leaves one
                        frame = CanFrame(header|0x0C_000_000, dataSegments[index])
                        self.link.sendCanFrame( frame )
                    
                
                #    send last one
                frame = CanFrame(header|0x0D_000_000, dataSegments[len(dataSegments) - 1])
                self.link.sendCanFrame( frame )
            ...
            if alias != None : #    might not know it if error
                header |= (alias & 0xFFF)
            else :
            ...
            if (msg.isAddressed()) :
                ...
                if destt == None : dest = NodeID(0)
                alias = self.nodeIdToAlias[ destt ]
                if alias != None  : #    might not know it?
                    #    address and have alias, break up data
                    dataSegments = self.segmentAddressedDataArray(alias, msg.data)
                    ...
                    logging.warning("Oon't know alias for destination = \(msg.destination ?? NodeID(0))")
    ...
    def segmentDatagramDataArray(self, data) :
        nSegments = int((len(data)+7) / 8) #    the +7 is since integer division takes the floor value
        if (nSegments == 0 ) :
            return [[]]
    ...
    def segmentAddressedDataArray(self, alias, data) :
        part0 = ( (alias >> 8) & 0xF)
        part1 = ( alias & 0xFF )
        nSegments = int((len(data)+5) / 6) #    the +5 is since integer division takes the floor value
    ...
    def checkAndHandleAliasCollision(self, frame) :
        if self.state != LinkLayer.State.Permitted : return False 
        receivedAlias = frame.header & 0x0000_FFF
        abort = (receivedAlias == self.localAlias)
        if (abort ) :
            #    Collision!
            logging.warning("alias collision in {}, we restart with AMR and attempt to get new alias".format(frame))
            self.link.sendCanFrame( CanFrame(ControlFrame.AMR.value, self.localAlias, self.localNodeID.toArray()) )
            #    Standard 6.2.5
            self.state = LinkLayer.State.Inhibited
    ...
    def decodeControlFrameFormat(self, frame) : 
        if (frame.header & 0x0800_0000) == 0x0800_0000 : #    data case; not checking leading 1 bit
            ...
            try: 
                ...
                logging.warning("Could not decode header 0x{:08X}".format(frame.header))
    ...
    #    class that holds the ID for accumulating a mult-part message:
    #      - MTI
    #      - Source
    #      - Destination
    ...
    class AccumKey : 
        ...
        def __eq__(self, other) :
            if self.mti != other.mti : return False
