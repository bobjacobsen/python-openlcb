...
class CanPhysicalLayerGridConnectTest(unittest.TestCase) :
    ...
    def testVerifyNodeSent(self) :
        ...
        gc.sendCanFrame(CanFrame(0x19170, 0x365, [0x02, 0x01, 0x12, 0xFE, 0x05, 0x6C]))
    ...
    def testOneFrameReceivedExactlyHeaderOnly(self) :
        ...
        bytes = [0x3a, 0x58, 0x31, 0x39, 0x34, 0x39, 0x30, 0x33, 0x36, 0x35, 0x4e, 0x3b, 0x0a] # :X19490365N;
    ...
    def testOneFrameReceivedExactlyWithData(self) :
        gc = CanPhysicalLayerGridConnect( self.captureString)
        gc.registerFrameReceivedListener(self.receiveListener)
        bytes = [0x3a, 0x58, 0x31, 0x39, 0x31, 0x42, 0x30, 0x33, 0x36, 0x35, 0x4e, 0x30,
                               0x32, 0x30, 0x31, 0x31, 0x32, 0x46, 0x45, 0x30, 0x35, 0x36, 0x43, 0x3b]
                               # :X19170365N020112FE056C;
        
        gc.receiveChars(bytes)
        
        self.assertEqual(self.receivedFrames[0], CanFrame( 0x191B0365, [0x02, 0x01, 0x12, 0xFE, 0x05, 0x6C]))
    ...
    def testOneFrameReceivedInTwoChunks(self) :
        gc = CanPhysicalLayerGridConnect( self.captureString)
        gc.registerFrameReceivedListener(self.receiveListener)
        bytes1 = [0x3a, 0x58, 0x31, 0x39, 0x31, 0x37, 0x30, 0x33, 0x36, 0x35, 0x4e, 0x30]
                                # :X19170365N020112FE056C;
        
        gc.receiveChars(bytes1)
        
        bytes2 = [0x32, 0x30, 0x31, 0x31, 0x32, 0x46, 0x45, 0x30, 0x35, 0x36, 0x43, 0x3b]
        gc.receiveChars(bytes2)

        self.assertEqual(self.receivedFrames[0], CanFrame( 0x19170365, [0x02, 0x01, 0x12, 0xFE, 0x05, 0x6C]))
