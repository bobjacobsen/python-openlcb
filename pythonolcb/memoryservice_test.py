...
class TestMemoryServiceClass(unittest.TestCase) :
    ...
    def testSingleRead(self) :
        ...
        self.assertEqual(len(LinkMockLayer.sentMessages), 2) # read reply datagram reply sent
    ...
    def testSingleWrite(self) : 
        ...
        self.dService.process(Message(MTI.Datagram_Received_OK, NodeID(123), NodeID(12)))
        self.assertEqual(len(LinkMockLayer.sentMessages), 1) # memory request datagram sent
        self.assertEqual(LinkMockLayer.sentMessages[0].data, [0x20, 0x01, 0,0,0,0, 1,2,3])
        self.assertEqual(len(self.returnedMemoryWriteMemo), 0) # no memory write op returned
        
        self.dService.process(Message(MTI.Datagram, NodeID(123), NodeID(12), [0x20, 0x11, 0,0,0,0]))
        self.assertEqual(len(LinkMockLayer.sentMessages), 2) # write reply datagram reply sent
        self.assertEqual(len(self.returnedMemoryWriteMemo), 1) # memory write returned
    ...
    def testMultipleRead(self) :
        ...
        self.assertEqual(len(LinkMockLayer.sentMessages), 1) # only one memory request datagram sent
        ...
        self.dService.process(Message(MTI.Datagram, NodeID(123), NodeID(12), [0x20, 0x51, 0,0,0,64, 1,2,3,4]))
