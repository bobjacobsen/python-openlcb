...
class TestLocalNodeProcessorClass(unittest.TestCase) :
    ...
    def testLinkUp(self) :
        ...
        self.assertEqual(LinkMockLayer.sentMessages[0],
                       Message(MTI.Initialization_Complete, self.node21.id, self.node21.id.toArray()))
    ...
    def testVerifyGlobal(self) :
        # not related to node
        msg1 = Message(MTI.Verify_NodeID_Number_Global, NodeID(13), None, [0,0,0,0,12,21])
        ...
        # global this Node ID
        msg3 = Message(MTI.Verify_NodeID_Number_Global, NodeID(13), None, [0,0,0,0,0,21])
        self.processor.process(msg3, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)

    def testVerifyAddressed(self) :
        # not related to node
        msg1 = Message(MTI.Verify_NodeID_Number_Addressed, NodeID(13), NodeID(24), [0,0,0,0,0,24])
        self.processor.process(msg1, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 0)

        # addressed no node ID
        msg2 = Message(MTI.Verify_NodeID_Number_Addressed, NodeID(13), NodeID(21))
        self.processor.process(msg2, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)
        LinkMockLayer.sentMessages = []
        
        # addressed this Node ID
        msg3 = Message(MTI.Verify_NodeID_Number_Addressed, NodeID(13), NodeID(21), [0,0,0,0,0,21])
        self.processor.process(msg3, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)
    ...
    def testPip(self) :
        ...
        self.processor.process(msg2, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)
        self.assertEqual(LinkMockLayer.sentMessages[0].data, [0x44,0x10,0x00, 0x00, 0x00, 0x00])
    ...
    def testSnip(self) :
        ...
        # not related to node
        msg1 = Message(MTI.Simple_Node_Ident_Info_Request, NodeID(13), NodeID(24))
        self.processor.process(msg1, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 0)

        # addressed to node
        msg2 = Message(MTI.Simple_Node_Ident_Info_Request, NodeID(13), NodeID(21))
        self.processor.process(msg2, self.node21)
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)
        self.assertEqual(LinkMockLayer.sentMessages[0].data[0:3], [0x04,0x53,0x61])
    ...
    def testUnsupportedMessageAddressed(self) :
        ...
        self.assertEqual(len(LinkMockLayer.sentMessages), 1)
        self.assertEqual(LinkMockLayer.sentMessages[0].mti, MTI.Optional_Interaction_Rejected)
        self.assertEqual(len(LinkMockLayer.sentMessages[0].data), 4)
        self.assertEqual(LinkMockLayer.sentMessages[0].data, [0x10, 0x43, 0x09, 0x48])  # error code, MTI

    def testDontRejectOIR(self) :
        msg1 = Message(MTI.Optional_Interaction_Rejected, NodeID(13), NodeID(21))
        self.processor.process(msg1, self.node21)
