...
class EventID :
    ...
    def __str__(self) :
        c = self.toArray()        
        return  "{:02X}.{:02X}.{:02X}.{:02X}.{:02X}.{:02X}.{:02X}.{:02X}".format(c[0], c[1], c[2], c[3], c[4], c[5], c[6], c[7])
    ...
    def __init__(self, data) :
        if isinstance(data, int) :  # create from an integer value
            ...
            self.eventId = 0
            if (len(data) > 0) : self.eventId |= (data[0] & 0xFF) << 56
            if (len(data) > 1) : self.eventId |= (data[1] & 0xFF) << 48
            if (len(data) > 2) : self.eventId |= (data[2] & 0xFF) << 40
            if (len(data) > 3) : self.eventId |= (data[3] & 0xFF) << 32
            if (len(data) > 4) : self.eventId |= (data[4] & 0xFF) << 24
            if (len(data) > 5) : self.eventId |= (data[5] & 0xFF) << 16
            if (len(data) > 6) : self.eventId |= (data[6] & 0xFF) <<  8
            if (len(data) > 7) : self.eventId |= (data[7] & 0xFF)      
        else :
