...
class LocalNodeProcessor(Processor) :
    ...
    def process(self, message, givenNode = None) :
        if givenNode == None: node = self.node
        else: node = givenNode
    ...
    def linkUpMessage(self, message, node) :
        ...
        msgIC = Message( MTI.Initialization_Complete,  node.id,  node.id.toArray())
    ...
    def verifyNodeIDNumberGlobal(self, message, node) :
        if  not (len(message.data) == 0 or node.id == NodeID(message.data)) : return # not to us
        msg = Message( MTI.Verified_NodeID,  node.id, message.source,  node.id.toArray())
    ...
    def protocolSupportInquiry(self, message, node) :
        ...
        msg = Message( MTI.Protocol_Support_Reply, node.id,  message.source,  retval)
        self.linkLayer.sendMessage(msg)
    
    # private method
    def simpleNodeIdentInfoRequest(self, message, node) :
        msg = Message( MTI.Simple_Node_Ident_Info_Reply,  node.id,  message.source,  node.snip.returnStrings())
        self.linkLayer.sendMessage(msg)
    ...
    def unrecognizedMTI(self, message, node) :
        if message.isGlobal() : return  # unrecognized global messages are ignored
        
        # addressed messages get an OptionalInteractionRejected
        logging.info("received unexpected {}, send OIR".format(message))
        msg = Message( MTI.Optional_Interaction_Rejected,  node.id,  message.source,
                           [0x10, 0x43, ((message.mti.value>>8)&0xFF), (message.mti.value&0xFF)]) # permanent error
        self.linkLayer.sendMessage(msg)
